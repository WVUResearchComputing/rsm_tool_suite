...
#define DYNAMIC_SEED 0  /* USES A DIFFERENT SEED FOR EACH RUN BASED ON CLOCK TIME */
#define SAMPLE 0 /* SAMPLE PARTICLE POSITION/VELOCITY FOR VISUALIZATION OUTPUT */

double n = 7.5e14;      /* ATMOSPHERIC NUMBER DENSITY [#/m^3] */
...

{
 ...
 /************************** Random Number Generator Setup *******************/
  ...
  char line[1024];
#if PARALLEL
  sprintf(outfilename, "Cdout_%s%d.dat", zeros, rank);
#else /* PARALLEL */
  sprintf(outfilename, "Cdout.dat");
#endif /* PARALLEL */
  ...
  read_input(filename, &NUM_POINTS, X, &GSI_MODEL);
  ...
  for(i=0; i<NUM_POINTS; i++) {
    if(GSI_MODEL==0) {
      fscanf(lhs_file, "%lf %lf %lf %lf %lf %lf\n", 
	     &LHS_Umag[i], &LHS_theta[i], &LHS_phi[i], &LHS_Ts[i], &LHS_Ta[i], &LHS_Ep[i]);
    } else if(GSI_MODEL==1) {
      fscanf(lhs_file, "%lf %lf %lf %lf %lf %lf\n", 
	     &LHS_Umag[i], &LHS_theta[i], &LHS_phi[i], &LHS_Ts[i], &LHS_Ta[i], &LHS_Al[i]);
    } else {

      fscanf(lhs_file, "%lf %lf %lf %lf %lf %lf %lf\n", 
...
/* DETERMINE NUMBER OF FACETS FROM MESH FILE */
  ...
  for(i=0; i<NSURF; i++) {
    for(j=0; j<NSPECIES; j++) {
      ...
      s[i][j] = Ubulk[i]/Cmp[j];
      surf_flux[i][j] = n*X[j]*Cmp[j]*surf_area[i]*
	(exp(-s[i][j]*s[i][j]) + sqrt(M_PI)*s[i][j]*(1.0 + erf(s[i][j])))/(2.0*sqrt(M_PI));
  ...
  Cd = compute_Cd(Umag, theta, phi, Fx, Fy, Fz, proj_area, n, m_avg);
